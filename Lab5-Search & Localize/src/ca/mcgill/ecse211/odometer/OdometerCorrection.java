 ...
 * ColorSensor class Corrects the odometer values when crossing a line.
 * 
...
public class OdometerCorrection extends Thread {
	...
	/**
	 * Constructor of the OdometerCorrection class. Initialises the odometer
	 * correction.
	 * 
	 * @param sensor
	 *            ColorSensor in red mode.
	 * @param odo
	 *            Odometer used by the robot's navigation system.
	 */
	...
	/**
	 * Run method of the OdometerCorrection Calls the lineCrossed() in ColorSensor
	 * Waits for a line to be read Corrects if close to line, but not if close to
	 * crossing of lines Runs on a while(true) loop--->.wait() to stop and .notify()
	 * to resume
	 */
	@Override
	public void run() {
		...
		while (true) {
			if (doCorrection) {
				if (lightSensor.lineDetected()) { // does not return until a line is crossed
					...
					lineX = Math.round(x + dynamicTrack.getLightSensorDistance() * Math.sin(Math.toRadians(theta))
							/ Navigation.TILE_SIZE);
					lineY = Math.round(y + dynamicTrack.getLightSensorDistance() * Math.cos(Math.toRadians(theta))
							/ Navigation.TILE_SIZE);
